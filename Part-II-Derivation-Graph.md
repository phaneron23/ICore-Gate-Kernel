...
```
              ...
              ↓            ↓            ↓
        ┌─────────┐  ┌─────────┐  ┌─────────┐
        │   UCE   │  │   UCC   │  │   UCM   │  ← Constitutional
        │Epistemol.│  │Calculus │  │  Math   │    Sciences
        └────┬────┘  └────┬────┘  └────┬────┘    (within USC)
             └──────┬─────┘            │
                    ↓                  │
              ┌─────────┐              │
              │   UCL   │←─────────────┘  ← Constitutional
              │Language │                   Science of Expression
              └────┬────┘
                   ↓
              ┌─────────┐
              │   UCRS  │  ← Constitutional Reference
              │Reference│    (self-representation of USC)
              └────┬────┘
                   ↓
              ┌─────────┐
...
**Linearity with controlled branching.** The core chain is linear from Reality through USC. After USC, the constitutional sciences (UCE, UCC, UCM) branch from a common origin and converge at UCL. This reflects that epistemology, calculus, and mathematics are parallel disciplines that together produce language.
...
**UCRS is the Constitution's self-representation.** UCRS does not reference something external — it references USC itself. It is the Constitution's way of knowing where everything is within its own space.
...
| **D3: Merge requires justification** | When two layers converge (as UCE + UCC + UCM → UCL), all parent layers must be cited. |
